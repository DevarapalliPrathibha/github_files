...
class FindFileLocation:
    ...
    def insert_file_search_results(self, fileLocation, fileName, searchCount = 0):
        uploadObject.upload_file_results_todb(fileName, fileLocation, searchCount)      # Method search_forfile_indb is used to search file results in db.
...
#locationObject.find_file_in_all_drives(input()) # file search in drive
uploadObject = UploadFiles.UploadFilesToDB()
fileToSearch = input() # file search in DB
